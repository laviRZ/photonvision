 ...
 */
 ...
 public class RKNNPipeline extends CVPipeline<CVPipelineResult, RKNNPipelineSettings> {
   ...
   public RKNNPipeline(RKNNPipelineSettings settings) {
     ...
     this.classes = List.of("cone", "cube");
   ...
   protected CVPipelineResult process(Frame input_frame, RKNNPipelineSettings settings) {
     ...
     for (int i = 0; i < results.count; i++) {
       ...
       Imgproc.rectangle(processed, new Point(box.left, box.top), new Point(box.right, box.bottom), new Scalar(0, 0, 255), 2);
 
 
       var name = String.format("%s (%f)", detection.name, detection.prop);
       ...
       var target = new TrackedTarget(
         ...
         0,
         detection.prop,
         new TargetCalculationParameters(false, null, null, null, null, this.frameStaticProperties)
       );
     ...
     return new CVPipelineResult(System.nanoTime() - sumPipeNanosElapsed, fps, targetList, input_frame);
   }
 }